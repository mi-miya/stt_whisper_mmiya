...
- **2つの入力モード**:
  - **マニュアルモード**: ホットキーで録音開始/停止を手動制御
  - **VADモード**: 音声を自動検知して録音・文字起こし
...
このツールは**2つの入力モード**を搭載しています。使いたいモードのホットキーを押すだけで自動的に切り替わります。
...
**方法2: ホットキー**
- **`Ctrl` + `Alt` + `Shift` + `J`**（マニュアルモード用ホットキー）を押す
- 話し終わったらもう一度同じキーを押す → 文字がクリップボードに保存

#### モード2: VADモード（音声自動検知）

音声を自動的に検知して、話し始めると自動で録音開始、無音になると自動で停止します。

**ホットキー**
- **`Ctrl` + `Alt` + `Shift` + `K`**（VADモード用ホットキー）を押す
- アイコンが青色になったら、話すだけで自動的に録音・文字起こしされます
- 停止する場合は、もう一度同じキーを押す
...
| | 自動貼り付け | 変換後に自動で貼り付け |
| | 効果音 | 録音開始/終了時の効果音 |
| | VAD設定 | VADモード時の無音停止時間、感度、最小録音時間 |
| **詳細** | GPU使用 | GPUレイヤー数（NVIDIA GPU用） |
| | サンプルレート | 録音品質 |
| | 無音判定の閾値 | 無音検出の感度 |
| **認識精度** | 初期プロンプト | 認識精度向上のヒント |
| | 候補数/ビームサイズ | 詳細な認識パラメータ |
| **ホットキー** | マニュアルモード用 | マニュアルモードのショートカットキー |
| | VADモード用 | VADモードのショートカットキー |
